...
import { applyOptimisticMove } from "./boardDnd";
...
describe("applyOptimisticMove", () => {
  ...
  it("floats the moved ticket to the top of its new state's group", () => {
    ...
    // The moved ticket's modifiedAt is `now`, strictly newer than `older` — sortBoard
    // (composed downstream via groupByState) will place it first regardless of array order.
